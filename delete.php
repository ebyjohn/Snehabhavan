...
// Check if the form is submitted
    ...
    if (isset($_GET['delete_id'])) {
        ...
        $deleteQuery = "DELETE FROM participantdetails WHERE ParticipantID = $deleteID";
    
        // Check if the deletion was successful
        if (mysqli_query($conn, $deleteQuery)) {
            // Redirect to the same page to remove the delete_id parameter from the URL
            header("Location: ".$_SERVER['PHP_SELF']);
            exit();
        } else {
            echo "Error deleting participant record: " . mysqli_error($conn);
        }
    }
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo '<th>ParentsPension</th>';
        echo '<th>ParentsPensionAmount</th>';
        echo '<th>Sibling Salary</th>';
        echo '<th>Income Level</th>';
        echo '<th>Land Ownership</th>';
        echo '<th>Land Area</th>';
        echo '<th>House ownership</th>';
        echo '<th>House Area</th>';
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<td><a class="delete-button" href="javascript:void(0);" onclick="confirmDelete(' . $row['ParticipantID'] . ')">Delete</a></td>';
        ...
        echo '</div>';
        echo '</div>';
        echo '<a href="kottayamdwnld.php" class="download-button">Download</a>';
    } else {
...
function confirmDelete(participantID) {
    if (confirm("Are you sure you want to delete this participant?")) {
        window.location.href = "?delete_id=" + participantID;
    }
}
</script>
