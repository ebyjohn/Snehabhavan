...
// Query to count children with vocational training
$countSql = "SELECT COUNT(DISTINCT CONCAT(FatherName, '-', MotherName)) AS FamilyCount
FROM participantdetails 
WHERE parentsPensionAmount>0";
$countResult = mysqli_query($conn, $countSql);
                    
// Fetch the count
$countRow = mysqli_fetch_assoc($countResult);
$familyCount = $countRow['FamilyCount'];

// Query to fetch children with vocational training
/*$detailsSql = "SELECT panchayat,Name, COUNT(DISTINCT CONCAT(FatherName, '-', MotherName)) AS FamilyCount, 
GROUP_CONCAT(DISTINCT CONCAT(FatherName, ' &amp; ', MotherName) SEPARATOR '<br>') AS FamilyNames
FROM participantdetails 
WHERE ParentsPension = 'YES'
GROUP BY panchayat";
$detailsResult = mysqli_query($conn, $detailsSql);*/
$detailsSql = "SELECT panchayat, COUNT(*) AS FamilyCount
               FROM participantdetails 
               WHERE parentsPensionAmount>0
               GROUP BY panchayat";
$detailsResult = mysqli_query($conn, $detailsSql);
...
<a href="reports.php" class="backtoreports-button">REPORTS</a>
    ...
    <div class="card">
        <h2 class="card-title">Total number of parents having a pensionable job</h2>
        <h3 style="text-align: center;"><?php echo $familyCount;  ?></h3>
